...
class TestCoreProcessor(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_process_success(self):
        """正常系プロセスのテスト"""
        # Mock dependencies directly on the processor instance
        ...
        self.processor.gemini_client = MagicMock()
        # Mock generate_content as an async method
        self.processor.gemini_client.generate_content = AsyncMock(return_value={"success": True, "result": "processed_text"})
    ...
    async def test_process_api_failure(self):
        """APIエラー時のテスト"""
        self.processor.gemini_client = MagicMock()
        self.processor.gemini_client.generate_content = AsyncMock(return_value={
            "success": False, 
            "error": "api_error", 
