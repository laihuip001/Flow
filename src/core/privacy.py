...
class PrivacyScanner:
    ...
    def __init__(self):
        ...
        # Ordered by specificity (Specific -> Generic) to handle overlaps correctly.
        # Note: Order matters for 'mask' logic!
        patterns_source = [
             # API Keys & Secrets (High Specificity)
            ("AWS_KEY", r"AKIA[0-9A-Z]{16}"),
            ("API_KEY", r"(?:sk-|pk_|AIza|ghp_|gsk_|glpat-|xox[baprs]-|Bearer\s+)[a-zA-Z0-9_-]{20,}"),

            # ID Numbers (Long -> Short)
            ...
            # Generic Secrets (Low Specificity, Contextual)
            # Use (?i) inside pattern or compile with re.IGNORECASE?
            # Original code used mix. EMAIL uses [a-zA-Z].
            # PASSWORD used (?i).
            # To preserve exact behavior, we should compile individually with appropriate flags if needed.
            # But mostly case sensitive is fine for these, except PASSWORD.
            ("PASSWORD", r"(?i)(?:password|passwd|pwd|secret|token)\s*[=:]\s*['\"]?[^\s'\"]{8,}"),
        ]

        # Pre-compile patterns for performance (avoids cache lookup overhead)
        # Note: We do NOT use global re.IGNORECASE here to preserve original behavior per pattern,
        # unless strict case insensitivity is desired.
        # Original code used re.findall(pattern, text) which respects inline flags like (?i).
        self.compiled_patterns = [
            (name, re.compile(pattern)) for name, pattern in patterns_source
        ]
        ...
        # Pre-calculate uppercase keywords for faster checking
        self.sensitive_keywords_upper = [kw.upper() for kw in self.sensitive_keywords]

    def scan(self, text: str) -> dict:
        findings = {}

        # Iterative Scan (Faster than combined regex for this pattern set/engine)
        for p_type, pattern_obj in self.compiled_patterns:
            matches = pattern_obj.findall(text)
            if matches:
                # Use set to remove duplicates, list to serializable
