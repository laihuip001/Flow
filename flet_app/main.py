...
def process_direct(text: str, style: str) -> str:
    """Direct Gemini API call with PII masking (No FastAPI overhead)."""
    prompts = {
    ...
    try:
        ...
        # PII Masking: マスクしてAPIに送信、復元して返却
        from logic import mask_pii, unmask_pii
        ...
        result = response.text.strip() if response.text else "Error: Empty response"

        # PII Unmasking: 結果内のプレースホルダを復元
        if pii_mapping:
            result = unmask_pii(result, pii_mapping)
...
def main(page: ft.Page):
            ...
            ft.Container(
                content=ft.Column(
                    [
                        ...
                        ft.Text(
                            "• ローカル: http://localhost:8000", size=12, color=ft.Colors.WHITE70
                        ),
                        ft.Text(
    ...
    process_section = ft.Container(
        content=process_button, padding=ft.padding.symmetric(horizontal=16)
    )
                        ...
                        [
                            ft.Row(
                                [
                                    ...
                                    ft.Container(
                                        content=ft.Text(
                                            item["style"].upper(), size=10, color=ft.Colors.WHITE
                                        ),
                                        bgcolor="#6366f1",
    ...
    settings_view = ft.Container(
        content=ft.Column(
            [
                ...
                ft.Divider(height=20, color=ft.Colors.WHITE12),
                ft.ElevatedButton(
                    content=ft.Row([ft.Icon(ft.Icons.DELETE_OUTLINE), ft.Text("Clear History")]),
                    on_click=clear_history_click,
                    style=ft.ButtonStyle(bgcolor="#dc2626"),
                ),
                ft.ElevatedButton(
                    content=ft.Row([ft.Icon(ft.Icons.RESTART_ALT), ft.Text("Reset Onboarding")]),
    ...
    main_view = ft.Column(
        [
            ...
            ft.Container(
                content=ft.Text(
                    "AI Clipboard Pro", size=22, weight=ft.FontWeight.BOLD, color=ft.Colors.WHITE
                ),
                alignment=ft.Alignment(0, 0),
            ),
            ft.Container(height=16),
